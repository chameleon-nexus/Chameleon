...
Chameleon 不仅仅是又一个AI聊天窗口。它是一款强大的开源VS Code插件，能将您的编辑器转变为一个专业级、本地优先的AI驱动型笔记本（Notebook）。

Chameleon专为开发者、创作者和研究人员设计，旨在将AI的掌控权交还给您。它深度集成于您的工作流，允许您无缝连接任何第三方AI供应商（如OpenAI、Google Gemini、DeepSeek等），管理本地与云端模型，并在您最熟悉的VS Code环境中，构建完全属于您自己的、私有的AI工具链。
...
* **🎛️ 深度IDE集成**：如同VS Code原生功能，通过右键菜单、代码镜头、专属面板随时调用AI工具。
...
Chameleon的运行依赖于 `Claude Code` 和 `Claude Code Router`。我们提供了便捷的安装向导：
