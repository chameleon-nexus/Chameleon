...
export class CopilotWelcomePanel {
    public static currentPanel: CopilotWelcomePanel | undefined;
    ...
    public static createOrShow(extensionUri: vscode.Uri, extensionContext: vscode.ExtensionContext) {
        ...
        if (CopilotWelcomePanel.currentPanel) {
            CopilotWelcomePanel.currentPanel.panel.reveal(column);
            return;
        }

        const panel = vscode.window.createWebviewPanel(
            'chameleonCopilotWelcome',
            t('copilotWelcome.exclusiveMode') + ' - Chameleon',
        ...
        CopilotWelcomePanel.currentPanel = new CopilotWelcomePanel(panel, extensionUri, extensionContext);
    }
    ...
    private async launchCLI() {
        try {
            ...
            terminal.show();
            vscode.window.showInformationMessage(t('copilotWelcome.launchSuccess'));
        } catch (error) {
            vscode.window.showErrorMessage(`${t('copilotWelcome.launchFailed')}: ${(error as Error).message}`);
        }
    }

    private async checkDependencies() {
        try {
            console.log('[CopilotWelcomePanel] 开始依赖检测...');
            ...
            const dependencies = await Promise.all([
                ...
                checkNpmPackage('@github/copilot')
            ]);
            ...
            vscode.window.showErrorMessage(`${t('copilotWelcome.checkDepsFailed')}: ${(error as Error).message}`);
            this.panel.webview.postMessage({ 
                command: 'dependenciesChecked', 
                result: null,
    ...
    private async showSystemSettings() {
        try {
            ...
            vscode.window.showErrorMessage(`${t('copilotWelcome.openSystemSettingsFailed')}: ${(error as Error).message}`);
        }
    }
    ...
    private async backToNavigation() {
        try {
            ...
            vscode.window.showErrorMessage(`${t('copilotWelcome.backToNavFailed')}: ${(error as Error).message}`);
        }
    }
        ...
        try {
            ...
            switch (dependency) {
                ...
                case '@github/copilot':
                    ...
                    break;
                default:
                    vscode.window.showErrorMessage(`${t('copilotWelcome.unsupportedDependency')}: ${dependency}`);
                    return;
            }
            ...
            vscode.window.showErrorMessage(`${t('copilotWelcome.installDepsFailed')}: ${(error as Error).message}`);
        }
    }
    ...
    private _getHtmlTemplate(): string {
        ...
        const jsTranslations = {
            launchFailed: t('copilotWelcome.launchFailed'),
            checkDepsFailed: t('copilotWelcome.checkDepsFailed'),
            depsLoading: t('copilotWelcome.depsLoading'),
            openSettingsFailed: t('copilotWelcome.openSettingsFailed'),
            openSystemSettingsFailed: t('copilotWelcome.openSystemSettingsFailed'),
            backToNavFailed: t('copilotWelcome.backToNavFailed'),
            unsupportedDependency: t('copilotWelcome.unsupportedDependency'),
            installDepsFailed: t('copilotWelcome.installDepsFailed'),
            configured: t('copilotWelcome.configured'),
            notConfigured: t('copilotWelcome.notConfigured'),
            installed: t('copilotWelcome.installed'),
            notInstalled: t('copilotWelcome.notInstalled'),
            clickToConfigure: t('copilotWelcome.clickToConfigure'),
...
<head>
    ...
    <title>${t('copilotWelcome.title')}</title>
    <style>
        body {
...
<body>
    <button class="back-button" onclick="backToNavigation()">${t('copilotWelcome.backToNav')}</button>
    
    <div class="header">
        <div class="logo">🦎 Chameleon</div>
        <div class="subtitle">${t('copilotWelcome.exclusiveMode')}</div>
    </div>

    <div class="feature-grid">
        <!-- Copilot CLI Launcher -->
        <div class="feature-block">
            <div class="feature-title">${t('copilotWelcome.title')}</div>
            <div class="feature-desc">${t('copilotWelcome.desc')}</div>

            <div class="dependencies-status" id="dependenciesStatus">
                <div class="deps-title">${t('copilotWelcome.depsTitle')}</div>
                <div class="deps-loading" id="depsLoading">${t('copilotWelcome.depsLoading')}</div>
            ...
            <div class="feature-actions" style="margin-top: 15px;">
                 <button class="btn" onclick="launchCLI()">${t('copilotWelcome.launchTerminal')}</button>
                 <button class="btn btn-secondary" onclick="checkDependencies()">${t('copilotWelcome.recheckDeps')}</button>
            </div>
        </div>

        <!-- Copilot Settings -->
        <div class="feature-block" onclick="showSettings()" style="cursor: pointer;">
            <div class="feature-title">${t('copilotWelcome.settings')}</div>
            <div class="feature-desc">${t('copilotWelcome.settingsDesc')}</div>
        </div>

        <!-- System Settings -->
        <div class="feature-block" onclick="showSystemSettings()" style="cursor: pointer;">
            <div class="feature-title">${t('copilotWelcome.systemSettings')}</div>
            <div class="feature-desc">${t('copilotWelcome.systemSettingsDesc')}</div>
        </div>
    </div>
    ...
    public dispose() {
        CopilotWelcomePanel.currentPanel = undefined;
