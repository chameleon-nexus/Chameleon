...
function CompatibilitySection() {
    ...
    return (<Section_1.default id="compatibility" title="One AGENTS.md works across many agents" className="py-12" center maxWidthClass="max-w-3xl">
      ...
      <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 gap-4 items-stretch">
        {agents.map(({ name, url, from, imageSrc, imageSrcLight, imageSrcDark, }) => {
            return (<a key={name} href={url} target="_blank" rel="noopener noreferrer" className="flex w-full h-20 p-3 items-center rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-gray-800">
                 <div className="w-16 h-16 flex items-center justify-center mr-3">
                  {imageSrcLight && imageSrcDark ? (<>
                      <image_1.default src={imageSrcLight} alt={`${name} logo`} width={64} height={64} className="block dark:hidden"/>
                      <image_1.default src={imageSrcDark} alt={`${name} logo`} width={64} height={64} className="hidden dark:block"/>
                    </>) : imageSrc ? (<span aria-hidden className="block w-16 h-16 bg-gray-700 dark:bg-gray-400" style={{
                ...
                <div className="flex flex-col justify-center">
                  ...
                  {from ? (<span className="text-sm text-gray-500 dark:text-gray-400">
...
exports.default = CompatibilitySection;
//# sourceMappingURL=CompatibilitySection.js.map