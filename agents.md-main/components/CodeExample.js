...
exports.HERO_AGENTS_MD = void 0;
const react_1 = __importDefault(require("react"));
const CopyIcon_1 = __importDefault(require("./icons/CopyIcon"));
exports.HERO_AGENTS_MD = `# AGENTS.md
...
function parseMarkdown(md) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith("# ") || line.startsWith("## ") || line.startsWith("### ")) {
            elements.push(<div key={i} className="font-bold">
          {line}
        </div>);
        }
 ...
 */
function renderLineWithInlineCode(line) {
    const parts = line.split(/(`[^`]+`)/g);
    return parts.map((part, index) => {
        if (part.startsWith("`") && part.endsWith("`")) {
            // This is inline code
            return (<span key={index} className="bg-gray-200 dark:bg-gray-800 px-1 rounded">
          {part}
        </span>);
        }
        // Regular text
 ...
 */
function CodeExample({ code, href, compact = false, heightClass, centerVertically = false, }) {
    ...
    const content = (<>
      <div className="relative">
        <button onClick={copyToClipboard} className={`absolute right-3 p-2 rounded-md bg-transparent text-gray-800 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors z-10 cursor-pointer ${centerVertically ? "top-1/2 -translate-y-1/2" : "top-3"}`} aria-label="Copy to clipboard">
          {copied ? (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"/>
            </svg>) : (<CopyIcon_1.default className="w-4 h-4"/>)}
        </button>
        <pre className={`relative rounded-lg bg-white dark:bg-black text-gray-800 dark:text-gray-100 text-xs leading-6 overflow-x-auto p-4 ${centerVertically ? "flex items-center" : ""} ${heightClass
            ? heightClass
            : compact
                ? ""
                : "min-h-[250px] max-h-[500px]"} border border-gray-200 dark:border-gray-700 shadow-sm`}>
          <code>
            {parseMarkdown(md)}
          </code>
        </pre>
    ...
    return (<section className="px-6 pt-10 pb-24 bg-gray-50 dark:bg-gray-900/40">
